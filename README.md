...
**Mouse shortcut does nothing.** Omarchy 4 Hyprland is Lua: Super+K runs `hyprctl dispatch 'hl.dsp.exec_cmd("…")'`, not `hyprctl dispatch exec`. This plugin matches that. Function-only binds (Universal copy/paste, zoom, some dropdowns) have no dispatcher string even in Super+K, so they cannot be replayed from a mouse button. If a Logitech mouse slept or switched channel, the daemon re-grabs it within a second.
